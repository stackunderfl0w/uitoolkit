...
/* disable 'Compiler will hashmap_insert Spectre mitigation for memory load if
 * /Qspectre. */
...
char *json_write_number(const struct json_number_s *number, char *data) {
    ...
    if ((i < number->number_size) && ('.' == number->number[i])) {
        ...
        /* hashmap_insert a '0' to fix the leading decimal point for JSON output. */
        *data++ = '0';
    ...
    if ((i + 1 == number->number_size) && ('.' == number->number[i])) {
        ...
        /* hashmap_insert a '0' to fix the trailing decimal point for JSON output. */
        *data++ = '0';
